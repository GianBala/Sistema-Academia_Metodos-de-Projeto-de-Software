...
public class SqliteAtendenteRepository implements AtendenteRepository {
    ...
    public void save(Atendente atendente) {
        String sql = "INSERT INTO atendentes (nome, dt_nascimento, email, login, senha_hash) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            stmt.setString(4, atendente.getLogin());
            stmt.setString(5, atendente.getSenhaHash());
            stmt.executeUpdate();
            try (ResultSet keys = stmt.getGeneratedKeys()) {
                if (keys.next()) atendente.setId(keys.getLong(1));
            }
    ...
    public List<Atendente> findAll() {
        String sql = "SELECT id, nome, dt_nascimento, email, login, senha_hash FROM atendentes";
        List<Atendente> atendentes = new ArrayList<>();
        try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) atendentes.add(mapRow(rs));
        } catch (SQLException e) {
    ...
    public Optional<Atendente> findById(long id) {
        String sql = "SELECT id, nome, dt_nascimento, email, login, senha_hash FROM atendentes WHERE id = ?";
    ...
    public Optional<Atendente> findByLogin(String login) {
        String sql = "SELECT id, nome, dt_nascimento, email, login, senha_hash FROM atendentes WHERE login = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            throw new RuntimeException("Erro ao buscar atendente por login.", e);
        }
        return Optional.empty();
    }
    ...
    public void update(Atendente atendente) {
        String sql = "UPDATE atendentes SET nome = ?, dt_nascimento = ?, email = ?, login = ?, senha_hash = ? WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, atendente.getNome());
            stmt.setString(2, atendente.getDataNascimento().toString());
            stmt.setString(3, atendente.getEmail());
            stmt.setString(4, atendente.getLogin());
            stmt.setString(5, atendente.getSenhaHash());
            stmt.setLong(6, atendente.getId());
            stmt.executeUpdate();
        } catch (SQLException e) {
